...
export class TransactionActions extends PayrexxActions<any, any> {
    ...
    public get(id:number, single = true) {
        ...
        return axios.get(this.getEndPoint(`${single ? id : ''}/`) + '&' + stringify(params))
    ...
    public capture(id, params = {}) {
        params['ApiSignature'] = this.rex.auth.buildSignature('');
        return axios.post(this.getEndPoint(`${id}/capture`), {data: stringify(params)})
    ...
    public refund(id, params = {}) {
        params['ApiSignature'] = this.rex.auth.buildSignature('');
        return axios.post(this.getEndPoint(`${id}/refund`), {data: stringify(params)})
            .then(response => this.successHandler(response, 'create'))
            .catch(err => this.errorHandler(err))
    ...
    public delete(id:number) {
        let params = {};
        params['ApiSignature'] = this.rex.auth.buildSignature('');
        return axios.delete(this.getEndPoint(`${id}/capture`), {data: stringify(params)})
